...
function MenuBackdrop() {
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 bg-[url('/media/noise.webp')] bg-size-[128px_128px] bg-repeat opacity-[0.54] mix-blend-soft-light" />
    </div>
  );
}
          ...
          {socialLinks.map(({ label, href, Icon }) => (
            <li key={label}>
              <a
                ...
                onClick={onClose}
                className="font-dm flex items-center gap-2.5 rounded p-2 text-base leading-6 font-semibold text-white/75 transition-colors hover:text-white"
              >
